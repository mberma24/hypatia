...
def process_folder(folder, pre_path):
    ...
    if folder == "Hash":
        ...
        # Calculate mean of non-zero deflections for each time_s
        df_avg = df[df["deflections"] != 0].groupby("time_s")["deflections"].mean().reset_index()
...
def main():
    ...
    for folder in folders:
        ...
        output_file = f"{folder}_deflections.txt"
        with open(output_file, "w") as f:
            for t, mean in zip(x, y_mean):
                f.write(f"{t}\t{mean}\n")
        print(f"Deflections data for {folder} written to {output_file}")
    ...
    plt.legend()
    plt.xlabel("Time (s)")
    plt.ylabel("Average deflections (log scale)")
    plt.title("Path Lengths for m_paths_1215 Across Folders (Mean ± Std Dev, Hash = single run)")
    
    graph_path = "single_change_cache_folders_1215_runs.png"
