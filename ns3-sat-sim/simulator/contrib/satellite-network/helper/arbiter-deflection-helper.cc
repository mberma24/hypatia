...
#include "arbiter-deflection-helper.h"

namespace ns3 {

ArbiterDeflectionHelper::ArbiterDeflectionHelper (Ptr<BasicSimulation> basicSimulation, NodeContainer nodes) {
    ...
    for (size_t i = 0; i < m_nodes.GetN(); i++) {
        Ptr<ArbiterDeflection> arbiter = CreateObject<ArbiterDeflection>(m_nodes.Get(i), m_nodes, initial_forwarding_state[i]);
...
void ArbiterDeflectionHelper::UpdateForwardingState(int64_t t) {
    ...
    if (!parse_boolean(m_basicSimulation->GetConfigParamOrDefault("satellite_network_force_static", "false"))) {
        ...
        if (next_update_ns < m_basicSimulation->GetSimulationEndTimeNs()) {
            Simulator::Schedule(NanoSeconds(m_dynamicStateUpdateIntervalNs), &ArbiterDeflectionHelper::UpdateForwardingState, this, next_update_ns);
        }
