...
#include "arbiter-gs-priority-deflection-helper.h"

namespace ns3 {

ArbiterGSPriorityDeflectionHelper::ArbiterGSPriorityDeflectionHelper (Ptr<BasicSimulation> basicSimulation, NodeContainer nodes) {
    std::cout << "SETUP LAST HOP DEFLECTION ROUTING" << std::endl;
    ...
    for (size_t i = 0; i < m_nodes.GetN(); i++) {
        Ptr<ArbiterGSPriorityDeflection> arbiter = CreateObject<ArbiterGSPriorityDeflection>(m_nodes.Get(i), m_nodes, initial_forwarding_state[i]);
...
ArbiterGSPriorityDeflectionHelper::InitialEmptyForwardingState() {
    std::vector<std::vector<std::vector<std::tuple<int32_t, int32_t, int32_t>>>> initial_forwarding_state;
    for (size_t i = 0; i < m_nodes.GetN(); i++) {
        std::vector<std::vector<std::tuple<int32_t, int32_t, int32_t>>> next_hop_matrix;
        for (size_t j = 0; j < m_nodes.GetN(); j++) {
            std::vector<std::tuple<int32_t, int32_t, int32_t>> next_hop_list;
            // You can choose the initial size of next_hop_list, here just one invalid entry as an example:
            next_hop_list.push_back(std::make_tuple(-2, -2, -2));
            next_hop_matrix.push_back(next_hop_list);
        }
        initial_forwarding_state.push_back(next_hop_matrix);
    }
    return initial_forwarding_state;
}

void ArbiterGSPriorityDeflectionHelper::UpdateForwardingState(int64_t t) {
    ...
    if (fstate_file) {
        ...
        while (getline(fstate_file, line)) {
            ...
            std::vector<std::string> comma_split = split_string(line, ",");
            // Parse node ids
            int64_t current_node_id = parse_positive_int64(comma_split[0]);
            int64_t target_node_id = parse_positive_int64(comma_split[1]);

            // Parse next hop triplets
            std::vector<std::tuple<int32_t, int32_t, int32_t>> next_hops;
            for (size_t i = 2; i + 2 < comma_split.size(); i += 3) {
                ...
                next_hops.emplace_back(next_hop_node_id, my_if_id + 1, next_if_id + 1); // Skip the loop-back interface
            ...
            m_arbiters.at(current_node_id)->SetDeflectionState(target_node_id, next_hops);
            ...
            // // Add to forwarding state
            // m_arbiters.at(current_node_id)->SetDeflectionState(
            //         target_node_id,
            //         next_hop_node_id,
    ...
    if (!parse_boolean(m_basicSimulation->GetConfigParamOrDefault("satellite_network_force_static", "false"))) {
        ...
        if (next_update_ns < m_basicSimulation->GetSimulationEndTimeNs()) {
            Simulator::Schedule(NanoSeconds(m_dynamicStateUpdateIntervalNs), &ArbiterGSPriorityDeflectionHelper::UpdateForwardingState, this, next_update_ns);
        }
